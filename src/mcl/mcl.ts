...
export function hashToPoint(msg: string, domain: Domain): MessagePoint {
  ...
  const _msg = ethers.getBytes(msg);
...
export function mapToPoint(e0: BigNumberish): mclG1 {
  let e1 = new mcl.Fp();
  //@todo not sure if it works as expected
  // e1.setStr(e0.mod(FIELD_ORDER).toString());
  e1.setStr((ethers.toBigInt(e0) % FIELD_ORDER).toString());
  return e1.mapToG1();
}
...
export function signValidatorMessage(
  domain: Domain,
  chainId: number,
  address: string,
  secret: SecretKey,
...
export function loadG1(hex: string): solG1 {
  const bytesarray = ethers.getBytes(hex);
  if (bytesarray.length != 64)
    throw new Error(`Expect length 64 but got ${bytesarray.length}`);
  const x = ethers.hexlify(bytesarray.slice(0, 32));
  const y = ethers.hexlify(bytesarray.slice(32));
  return [x, y];
}
...
export function loadG2(hex: string): solG2 {
  ...
  const x0 = ethers.hexlify(bytesarray.slice(0, 32));
  const x1 = ethers.hexlify(bytesarray.slice(32, 64));
  const y0 = ethers.hexlify(bytesarray.slice(64, 96));
  const y1 = ethers.hexlify(bytesarray.slice(96, 128));
  return [x0, x1, y0, y1];
}
